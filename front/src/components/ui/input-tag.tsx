...
const InputTags = React.forwardRef<HTMLInputElement, InputTagsProps>(
  ({ className, value, onChange, ...props }, ref) => {
    ...
    return (
      ...
      >
        {value.map(item => (
          <Badge key={item} variant='tag_input'>
            {item}
            <Button
              variant='ghost'
              size='icon'
        ...
        <input
          className='flex-1 w-full dark:outline-none rounded-md bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50'
